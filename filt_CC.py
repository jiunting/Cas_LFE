...
#------CC level-------
CC = 0.2
dir_data = '/projects/amt/jiunting/Cascadia_LFE/Data_QC_rmean'


# output dir. make dir if dir not exist
#dir_out = '/projects/amt/jiunting/Cascadia_LFE/Data_QC_rmean_CC_%3.1f/'%(CC)
#if not(os.path.exists(dir_out)):
#    os.makedirs(dir_out)


# output dir. make dir if dir not exist
dir_out = '/projects/amt/jiunting/Cascadia_LFE/Data_QC_rmean_NotCC_%3.1f/'%(CC)
if not(os.path.exists(dir_out)):
    os.makedirs(dir_out)


#read clean QC data, stack them and calculate CC for every trace
...
for i_data in all_data:
    ...
    data_stack = np.mean(data['waves'],axis=0) #mean is basically stacking
    print('stacked data shape=',data_stack.shape)
    idx = []
    for i,i_trace in enumerate(data['waves']):
        #if cal_CCC(i_trace,data_stack)>=CC:
        if cal_CCC(i_trace,data_stack)<CC: #find those with small CC value
            idx.append(i) 
    idx = np.array(idx)
    print('original length=',len(data['waves']))
    print('new length=',len(idx))
    if len(idx)>0:
        data_filt_CC = data['waves'][idx,:]
        #save the filtCC result (without normalization)
        h5f = h5py.File(dir_out+'/'+fileName,'w')
        h5f.create_dataset('waves',data=data_filt_CC)
        h5f.close()
    data.close()
