...
#filename = './test.nlloc'
filt_N = 3 # minimum number of observations 

sav_sta_phase = []
sav_t_part = [] # only have partial t, not the full sav_t
#sav_table_obs = []

OUT1 = open('family_classify_S_y0.1.txt','w')
    ...
    for line in IN1:
        if len(line.split())!=14:
            ...
            if len(sav_sta_phase)!=0:
                match_fam(sav_sta_phase,sav_t_part,OUT1) # this done all the work below
                '''
                # deal with previous stuff
                sav_sta_phase = np.array(sav_sta_phase)
                ...
                sav_mean = [] #mean of the misfit*2 value
                ...
                idx_gt1sta = np.where(sav_n_obs>=filt_N)[0] # remove 0 and 1(itself) station match
                if len(idx_gt1sta)==0:
                    # not enough stations left
                    OUT1.write('%s  %f  %s  %d  %d\n'%('000',0,"2000-01-01T00:00:00.00",0,sav_n_obs.max()))
                    continue
                sav_mean = sav_mean[idx_gt1sta]
                sav_n_obs = sav_n_obs[idx_gt1sta]
                tmp_table_fam = sav_table_fam[idx_gt1sta]
