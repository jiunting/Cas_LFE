### convert daily timeseries .mseed data to model input and run model###
"""
Modified from `daily2input_S_20220919.py` for PB detection
directory = /projects/amt/shared/cascadia_PB/data/PB/YYYY/DOY
filename = directory/BXXX.PB.YYYY.DOY
...
def data_process(D,filePath,sampl=100):
    '''
        Same function but for PB
        load and process daily .mseed data
        filePath: absolute path of .mseed file.
        sampl: sampling rate
        other processing such as detrend, taper, filter are hard coded in the script, modify them accordingly
    ...
    #t2 = D[0].stats.endtime
    # t1, t2 are not necessarily a whole day. Get the t1,t2 from file name instead
    DOY = filePath.split('.')[-1]
    YYYY = filePath.split('.')[-2]
    t1 = UTCDateTime(YYYY+'0101') + (int(DOY)-1) * 86400
    t2 = t1 + 86400
    D.detrend('linear')
    D.taper(0.02) #2% taper
    D.filter('highpass',freq=1.0)
...
def detc_sta(sta):
    ...
    #--- Search all the available data based on station name ---
    filepaths = []
    dir_yrs = glob.glob("/projects/amt/shared/cascadia_PB/data/PB/*")
    dir_yrs.sort()
    for dir_yr in dir_yrs:
        DOYs = glob.glob(dir_yr+"/*")
        DOYs.sort()
        for DOY in DOYs:
            filepath = DOY+"/"+sta+".PB."+dir_yr.split('/')[-1]+"."+DOY.split('/')[-1]
    ...
    #file_csv = './Detections_S_new/cut_daily_%s.%s.csv'%(net,sta)  #the previous Detections_S_new has renamed to Detections_S_addSta i.e. additional station detc
    #file_hdf5 = './Detections_S_new/cut_daily_%s.%s.hdf5'%(net,sta)
    #file_csv = './Detections_S_C8_new/cut_daily_%s.%s.csv'%(net,sta)  #the previous Detections_S_new has renamed to Detections_S_addSta i.e. additional station detc
    #file_hdf5 = './Detections_S_C8_new/cut_daily_%s.%s.hdf5'%(net,sta)
    file_csv = './Detections_S_PB/cut_daily_%s.%s.csv'%(net,sta)  #the previous Detections_S_new has renamed to Detections_S_addSta i.e. additional station detc
    file_hdf5 = './Detections_S_PB/cut_daily_%s.%s.hdf5'%(net,sta)
    ...
    for D_Z in D_Zs:
        ...
        #hf = h5py.File(file_hdf5,'a')
        print('--currently at:',D_Z_file)
        D_Z = obspy.read(D_Z)
        # The miniseed file has 3-components: Z12
        D_Z.merge()
        if len(D_Z)!=3:
            print('Missing at least one component data: %s'%(D_Z_file))
            OUT1.close()
            continue
        # ENZ or 12Z
        D_E = D_Z.select(component='E')
        if len(D_E)==0:
            D_E = D_Z.select(component='1')
        D_N = D_Z.select(component='N')
        if len(D_N)==0:
            D_N = D_Z.select(component='2')
        # Final check if all 3 components exist
        if (len(D_E)==0) or (len(D_N)==0):
            print('File %s has unexpected component'%(D_Z_file))
            OUT1.close()
            continue # not ENZ nor 12Z, what can it be??
        D_Z = D_Z.select(component='Z')
        # process the 3C data to be exact 1 day, detrend and high-pass
        try:
            D_Z = data_process(D_Z,D_Z_file,sampl=100) # sampling rate = 100
            D_E = data_process(D_E,D_Z_file,sampl=100)
            D_N = data_process(D_N,D_Z_file,sampl=100)
        except:
            print('Cannot process data')
            #hf.close()
            OUT1.close()
            continue
        chn = D_Z[0].stats.channel[:2]
        times = D_Z[0].times()
...
all_sta = ['B001','B003','B004','B007','B013','B014','B017','B018','B019','B020','B022','B023','B024','B028','B030','B031','B033','B036',]
n_cores = len(all_sta)
