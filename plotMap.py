...
sav_eqloc = []
sav_ID = []
OUT2 = open('lfe_fam_loc.txt','w')
for ID in EQinfo.keys():
    sav_eqloc.append(EQinfo[ID]['eqLoc'])
    OUT2.write('%s %f %f %f\n'%(ID,EQinfo[ID]['eqLoc'][0]*-1,EQinfo[ID]['eqLoc'][1],EQinfo[ID]['eqLoc'][2]))
    
OUT2.close()
sav_eqloc = np.array(sav_eqloc)
sav_eqloc[:,0] = sav_eqloc[:,0]*-1




#detc = pd.read_csv('cut_daily_all.csv')
detcs = glob.glob('./Detections_S/*.csv') #./Detections_S/cut_daily_CN.GOBB.csv
...
plt.scatter(sav_eqloc[:,0],sav_eqloc[:,1],c=sav_eqloc[:,2],cmap=plt.cm.jet)
...
OUT1 = open('vancouver_station_list.txt','w')
for i,sta in enumerate(sav_stname):
    OUT1.write('%s %s %f %f\n'%(sav_net[i],sta,sav_stlat[i],sav_stlon[i]))
