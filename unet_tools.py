...
def my_3comp_data_generator(batch_size,x_data,n_data,sig_inds,noise_inds,sr,std,valid=False):
    epsilon=1e-6
    while True:
        ...
        nlen=int(sr*30)+1
        # grab batch
        comp1=np.concatenate((x_data['waves'][datainds,:nlen],n_data['noise'][noiseinds,:nlen]))
        comp2=np.concatenate((x_data['waves'][datainds,nlen:2*nlen],n_data['noise'][noiseinds,nlen:2*nlen]))
        comp3=np.concatenate((x_data['waves'][datainds,2*nlen:],n_data['noise'][noiseinds,2*nlen:]))
