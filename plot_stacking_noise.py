...
data_path = '/projects/amt/jiunting/Cascadia_LFE/Noise_QC_rmean/'
data_all_noise = glob.glob(data_path+'*.h5') # e.g. CN.SNB.BH_noise.h5
...
stack_max = 5000
for d in data_all_noise:
    print('Now loading:',d)
    #fam = d.split('/')[-1].split('_')[1]
    net_sta = '.'.join([d.split('/')[-1].split('.')[0],d.split('/')[-1].split('.')[1] ])
    #phase = d.split('/')[-1].split('_')[3].split('.')[0] #P or S
    dtfl = h5py.File(d, 'r')
    A = dtfl.get('waves')
    AA = np.array(A)
    np.random.seed(99)
    np.random.shuffle(AA) #shuffle the array
    ...
    axs[0].text(2,n_plt+0.5,'stack (N=%d)'%(min(stack_max,i+1)),fontsize=12)
    ...
    axs[0].set_title('%s'%(net_sta),fontsize=15)
    # plot 2nd subplot
    ...
    axs[1].plot(T[:3001],np.zeros_like(T[:3001]),'b-') # plot gaussian
    ...
    fig.savefig('./Figs_noise_stacking/noise_%s.png'%(net_sta))
    #fig.close()
    plt.close()
    dtfl.close()
    #break
